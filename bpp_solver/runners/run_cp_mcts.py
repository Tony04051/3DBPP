...
from ..CP.MCTS.mc_packer import MCTS_Packer
from config import *
...
def perform_single_simulation(initial_item_list: list[Item], num_simu: int):
    """
    執行一次完整的裝箱模擬。
    ...
    mcts_packer = MCTS_Packer(num_simu)
    ...
    while conveyor_items or temp:
        ...
        if not candidate_items:
            break
        t0 = time.time()
        best_placement = mcts_packer.pack(cage, candidate_items)
        t1 = time.time()
        if best_placement:
...
def run_experiment(num_datasets: int, num_shuffles: int, args):
    """
    執行完整的隨機實驗並報告結果。
    """
    print("="*60)
    ...
    for i in range(num_datasets):
        ...
        for j in range(num_shuffles):
            ...
            result = perform_single_simulation(shuffled_items, num_simu)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="3D Bin Packing Problem Solver (EMS + MCTS)")
    parser.add_argument('--num_simu', type=int, default=100, help='Number of MCTS simulations per action.')
    args = parser.parse_args()
    run_experiment(NUM_DATASETS, NUM_SHUFFLES_PER_DATASET, args)