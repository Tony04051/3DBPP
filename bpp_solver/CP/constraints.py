...
def is_placement_valid(
    ...
    if not check_weight_constraint(cage, item):
        counter += 1 
    if not check_stackable_constraint(cage.packed_items, item, position, rotated_dims):
        counter += 1
...
def check_weight_constraint(cage: CageTrolley, item: Item) -> bool:
    """檢查是否超重"""
    return cage.current_weight + item.weight <= CAGE_WEIGHT_LIMIT
...
def check_stackable_constraint(
    ...
    # 由籠車底部支撐
    if abs(item_bottom_z - 0.0) < TOLERANCE:
        floor_rect = (0, 0, CAGE_DIMENSIONS[0], CAGE_DIMENSIONS[1]) # 假设能从config导入
        total_supported_area += get_intersection_area(item_footprint, floor_rect)
    ...
    for p_item in packed_items:
        ...
        if p_pos is None:
            continue
        p_top_z = p_pos[2] + p_dims[2]
        
        # 如果新物品的底部恰好在这个已放置物品的顶部
        if abs(item_bottom_z - p_top_z) < MERGE_MARGIN:
            # 支撐物品底面積
            p_top_rect = (p_pos[0], p_pos[1], p_pos[0] + p_dims[0], p_pos[1] + p_dims[1])
            # 交集
            total_supported_area += get_intersection_area(item_footprint, p_top_rect)
            
    required_area = item_bottom_area * STABILITY_FACTOR
    return total_supported_area >= required_area - TOLERANCE
...
def check_collision_constraint(
    ...
    z1_max_up = CAGE_DIMENSIONS[2] # 高度不變，從頂部插入
    # 2. 從一側面插入, 假設是x = (0, 100), y = 0
    x1_min, y1_min, z1_min = pos
    x1_max_s = x1_min + dims[0]
    y1_max_s = CAGE_DIMENSIONS[1]
...
def check_center_of_gravity_constraint(
    ...
    cage_l, cage_w, _ = CAGE_DIMENSIONS
